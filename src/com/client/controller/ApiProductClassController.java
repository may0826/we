...
public class ApiProductClassController {
	...
	public JsonResult update(@Validated ProductClass productClass, BindingResult br, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        return JsonResultBuilder.ok(productClassService.getProductClass(productClass.getProductClassId()));
	}

	/*ajax方式显示获取商品分类详细信息*/
	@RequestMapping(value="/get/{productClassId}",method=RequestMethod.POST)
	public JsonResult getProductClass(@PathVariable int productClassId,Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {
	...
	/*ajax方式删除商品分类记录*/
	@RequestMapping(value="/delete/{productClassId}",method=RequestMethod.POST)
	public JsonResult deleteProductClass(@PathVariable int productClassId,Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {
		//通过accessToken获取到用户信息 
		String userName = authService.getUserName(request);
		if(userName == null) return JsonResultBuilder.error(ReturnCode.TOKEN_VALID_ERROR);
		try {
