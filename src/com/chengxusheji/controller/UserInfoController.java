...
public class UserInfoController extends BaseController {
	...
	public void add(@Validated UserInfo userInfo, BindingResult br,
		...
		try {
			userInfo.setMyPhoto(this.handlePhotoUpload(request, "myPhotoFile"));
	...
	public void list(String user_name,String realname,String bornDate,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (user_name == null) user_name = "";
		if (realname == null) realname = "";
		if (bornDate == null) bornDate = "";
		if(rows != 0)userInfoService.setRows(rows);
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, realname, bornDate, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, realname, bornDate);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,String realname,String bornDate,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (user_name == null) user_name = "";
		if (realname == null) realname = "";
		if (bornDate == null) bornDate = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, realname, bornDate, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, realname, bornDate);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void update(@Validated UserInfo userInfo, BindingResult br,
		...
		if(!myPhotoFileName.equals("upload/NoImage.jpg"))userInfo.setMyPhoto(myPhotoFileName); 
	...
	public void OutToExcel(String user_name,String realname,String bornDate, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(realname == null) realname = "";
        if(bornDate == null) bornDate = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,realname,bornDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "手机用户名","姓名","性别","出生日期","用户qq","用户邮箱","用户头像"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getRealname(),userInfo.getSex(),userInfo.getBornDate(),userInfo.getQq(),userInfo.getEmail(),userInfo.getMyPhoto()});
        }
        /*
