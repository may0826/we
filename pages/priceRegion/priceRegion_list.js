...
Page({
  ...
  // 加载价格区间列表
  listPriceRegion: function () {
    var self = this
    var url = config.basePath + "api/priceRegion/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          priceRegions: self.data.priceRegions.concat(res.data.list),
  ...
  //删除价格区间记录
  removePriceRegion: function (e) {
    var self = this
    var regionId = e.currentTarget.dataset.regionid
    wx.showModal({
      title: '提示',
      content: '确定要删除regionId=' + regionId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/priceRegion/delete/" + regionId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除价格区间后客户端同步删除数据
            var priceRegions = self.data.priceRegions;
            for (var i = 0; i < priceRegions.length; i++) {
              if (priceRegions[i].regionId == regionId) {
                priceRegions.splice(i, 1)
                break
              }
            }
            self.setData({ priceRegions: priceRegions })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
