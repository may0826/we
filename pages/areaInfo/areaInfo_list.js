...
Page({
  ...
  // 加载区域信息列表
  listAreaInfo: function () {
    var self = this
    var url = config.basePath + "api/areaInfo/list"
    //如果要显示的页码超过总页码不操作
    ...
    utils.sendRequest(url, {
      ...
      setTimeout(function () {  
        self.setData({
          areaInfos: self.data.areaInfos.concat(res.data.list),
  ...
  //删除区域信息记录
  removeAreaInfo: function (e) {
    var self = this
    var areaId = e.currentTarget.dataset.areaid
    wx.showModal({
      title: '提示',
      content: '确定要删除areaId=' + areaId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/areaInfo/delete/" + areaId
          utils.sendRequest(url, null, function (res) {
            ...
            //删除区域信息后客户端同步删除数据
            var areaInfos = self.data.areaInfos;
            for (var i = 0; i < areaInfos.length; i++) {
              if (areaInfos[i].areaId == areaId) {
                areaInfos.splice(i, 1)
                break
              }
            }
            self.setData({ areaInfos: areaInfos })
          })
        } else if (sm.cancel) {
          console.log('用户点击取消')
        }
