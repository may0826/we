...
Page({
  ...
  removeProductClass: function (e) {
    var self = this
    var productClassId = e.currentTarget.dataset.productclassid
    wx.showModal({
      title: '提示',
      content: '确定要删除productClassId=' + productClassId + '的记录吗？',
      success: function (sm) {
        if (sm.confirm) {
          // 用户点击了确定 可以调用删除方法了
          var url = config.basePath + "api/productClass/delete/" + productClassId
          utils.sendRequest(url, null, function (res) {
            ...
            for (var i = 0; i < productClasss.length; i++) {
              if (productClasss[i].productClassId == productClassId) {
